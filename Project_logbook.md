...
### Step 4 - B Session Nonce for Replay Protection

To address the remaining replay at `B`, the final week-6 protocol now adds a
fresh session nonce `Nb` generated by `B` and signed into the delegation token.
This follows the lecture point that replay protection requires explicit
freshness at the point where replay can occur.

Updated flow:

1. `P -> B`: start an access session
2. `B -> P`: return fresh challenge `Nb`
3. `P -> A`: forward `Nb`
4. `A -> idp`: request a token over `(A,P,B,Scope,Na,Nb,Ka,pw(A,idp))`
5. `idp -> A`: sign a token containing the same `Nb`
6. `P -> B`: redeem the token together with `Nb`

Security rationale:
- `Ka` still protects the low-entropy password from offline guessing.
- `Nb` now binds the delegation token to one concrete access session at `B`.
- Replaying an old token in a later session fails because the new session uses
  a different `Nb`.